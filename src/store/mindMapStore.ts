...
export const validatePartialMindMapMeta =
    createSchemaValidator<Partial<MindMapMeta>>(PartialMeta_JsonSchema);
...
export const initMindMapStore = async () => {
    ...
    await withLoading('Loading Meta', async () => {
        ...
        if (
            meta.value.currentKey &&
            meta.value.mindMaps.findIndex((it) => it.key === meta.value.currentKey) !== -1
        ) {
...
export const useMindMapStore = createStore(() => {
    const add = async (index: number, name: string) => {
        return await withLoading('Create MindMap', async () => {
            try {
                ...
                meta.value.mindMaps.splice(index, 0, {
                    key,
                    name,
    ...
    const get = async (key: string): Promise<MindMapData> => {
        ...
        if (data === undefined) {
            sendMessage(
                `${translate('get_mindMap_fail')}\nMindMap [${item.name}] file not existed.`,
                {type: 'error'},
            );
        ...
        if (!validateMindMapData(parsedData)) {
            sendMessage(`${translate('get_mindMap_fail')}\nMindMap [${item.name}] data illegal.`, {
                type: 'error',
            });
